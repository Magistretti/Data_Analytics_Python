...
df_despachosS = pd.read_sql('''
    ...
    select count(distinct ID) as 'Ticadas en SC' from VIEW_VENTAS_POR_EMPLEADO
...
df_despachos = df_despachosS.merge(df_despachosS2, on=['Semana','Ticadas en SC'],how='outer')
df_despachos = df_despachos.merge(df_despachosS3,on=['Semana','Ticadas en SC'],how='outer')
df_despachos = df_despachos.merge(df_despachosS4,on=['Semana','Ticadas en SC'],how='outer')
...
df_redPagoS = pd.read_sql('''
	...
	from TransacEWallet_view AS W join VTCtaCte as D 
	on 
		 W.UEN = D.UEN
...
df_PenRedPago["Penetracion %"]= df_PenRedPago['Cant Op Con RedPago'] / df_PenRedPago['Ticadas en SC']

### Creo TOTALES

df_PenRedPago.loc["colTOTAL"]= pd.Series(
    df_PenRedPago.sum()
    , index=["Ticadas en SC","Cant Op Con RedPago"]
)
df_PenRedPago.fillna({"Semana":"TOTAL"}, inplace=True)

tasa = (df_PenRedPago.loc["colTOTAL","Cant Op Con RedPago"] /
        df_PenRedPago.loc["colTOTAL","Ticadas en SC"])
df_PenRedPago.fillna({"Penetracion %":tasa}, inplace=True)


df_PenRedPago=df_PenRedPago.reindex(columns=['Semana','Cant Op Con RedPago','Ticadas en SC','Penetracion %'])
...
df_PenRedPago = _estiladorVtaTituloP(df_PenRedPago,numCols0,numCols,percColsPen, "RedPago ServiCompras - Penetracion en Cantidad de Operaciones")



ubicacion = "C:/Informes/Red Pago//"
nombreDespachos = "PenRedPagoServiC.png"
