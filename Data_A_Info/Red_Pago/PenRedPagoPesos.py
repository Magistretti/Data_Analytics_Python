...
df_despachosS = pd.read_sql('''
    ...
    select  sum(Importe) as '$ Totales Despachados' from VIEW_DESPAPRO_FILTRADA
...
df_despachos = df_despachosS.merge(df_despachosS2, on=['Semana','$ Totales Despachados'],how='outer')
df_despachos = df_despachos.merge(df_despachosS3,on=['Semana','$ Totales Despachados'],how='outer')
df_despachos = df_despachos.merge(df_despachosS4,on=['Semana','$ Totales Despachados'],how='outer')
...
df_redPagoS = pd.read_sql('''
	...
	--de días del mes actual
		...
		select sum(D.importe) * -1 AS '$ Cobrados en Playa con Wallet'
	...
	where W.FECHATUR >= DATEADD(day, -7, CAST(GETDATE() AS date))
		...
		AND D.OBSERVACIONES LIKE '%REDPAGO%'
...
df_redPago = df_redPagoS.merge(df_redPagoS1, on=['Semana','$ Cobrados en Playa con Wallet'],how='outer')
df_redPago = df_redPago.merge(df_redPagoS2,on=['Semana','$ Cobrados en Playa con Wallet'],how='outer')
df_redPago = df_redPago.merge(df_redPagoS3,on=['Semana','$ Cobrados en Playa con Wallet'],how='outer')
...
df_PenRedPago["Penetracion %"]= df_PenRedPago['$ Cobrados en Playa con Wallet'] / df_PenRedPago['$ Totales Despachados']

### Creo TOTALES

df_PenRedPago.loc["colTOTAL"]= pd.Series(
    df_PenRedPago.sum()
    , index=["$ Totales Despachados","$ Cobrados en Playa con Wallet"]
)
df_PenRedPago.fillna({"Semana":"TOTAL"}, inplace=True)

tasa = (df_PenRedPago.loc["colTOTAL","$ Cobrados en Playa con Wallet"] /
        df_PenRedPago.loc["colTOTAL","$ Totales Despachados"])
df_PenRedPago.fillna({"Penetracion %":tasa}, inplace=True)


df_PenRedPago=df_PenRedPago.reindex(columns=['Semana','$ Cobrados en Playa con Wallet','$ Totales Despachados','Penetracion %'])
...
numCols0 = ["$ Cobrados en Playa con Wallet"
            , "$ Totales Despachados"
...
df_PenRedPago = _estiladorVtaTituloP(df_PenRedPago,numCols0,numCols,percColsPen, "RedPago Playa - Penetracion en $ de Operaciones")



ubicacion = "C:/Informes/Red Pago//"
nombreDespachos = "PenRedPagoPesos.png"
