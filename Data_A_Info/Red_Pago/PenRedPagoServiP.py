...
df_despachosS = pd.read_sql('''
    ...
    select  sum(Importe) as '$ Totales Servicompra' from VIEW_VENTAS_POR_EMPLEADO
...
df_despachos = df_despachosS.merge(df_despachosS2, on=['Semana','$ Totales Servicompra'],how='outer')
df_despachos = df_despachos.merge(df_despachosS3,on=['Semana','$ Totales Servicompra'],how='outer')
df_despachos = df_despachos.merge(df_despachosS4,on=['Semana','$ Totales Servicompra'],how='outer')
...
df_redPagoS = pd.read_sql('''
	...
	--de días del mes actual
		...
		select sum(D.importe) * -1 AS '$ Cobrados en ServiCompras con Wallet'
	from TransacEWallet AS W join VTCtaCte as D 
	on 
		 W.UEN = D.UEN
	...
	where d.FECHASQL >= DATEADD(day, -7, CAST(GETDATE() AS date))
		and	d.FECHASQL < @hoy
		and W.Departamento = 'SERVICOMPRAS        '
...
df_redPagoS1 = pd.read_sql('''
    ...
    And W.PtoVta = D.PTOVTA
    AND W.FECHATUR = D.FECHASQL
	where d.FECHASQL >= DATEADD(day, -14, CAST(GETDATE() AS date))
...
df_redPagoS2 = pd.read_sql('''
    ...
    And W.PtoVta = D.PTOVTA
    AND W.FECHATUR = D.FECHASQL
	where d.FECHASQL >= DATEADD(day, -21, CAST(GETDATE() AS date))
		and	d.FECHASQL < DATEADD(day, -14, CAST(GETDATE() AS date))
		and W.Departamento = 'SERVICOMPRAS        '
		AND W.TipoComp like 'FAC'
...
df_redPagoS3 = pd.read_sql('''
    ...
    And W.PtoVta = D.PTOVTA
    AND W.FECHATUR = D.FECHASQL
	where d.FECHASQL >= DATEADD(day, -28, CAST(GETDATE() AS date))
		and	d.FECHASQL < DATEADD(day, -21, CAST(GETDATE() AS date))
		and W.Departamento = 'SERVICOMPRAS        '
		AND W.TipoComp like 'FAC'
    ''' ,db_conex)
...
df_redPago = df_redPagoS.merge(df_redPagoS1, on=['Semana','$ Cobrados en ServiCompras con Wallet'],how='outer')
df_redPago = df_redPago.merge(df_redPagoS2,on=['Semana','$ Cobrados en ServiCompras con Wallet'],how='outer')
df_redPago = df_redPago.merge(df_redPagoS3,on=['Semana','$ Cobrados en ServiCompras con Wallet'],how='outer')
...
df_PenRedPago["Penetracion %"]= df_PenRedPago['$ Cobrados en ServiCompras con Wallet'] / df_PenRedPago['$ Totales Servicompra']

### Creo TOTALES

df_PenRedPago.loc["colTOTAL"]= pd.Series(
    df_PenRedPago.sum()
    , index=["$ Totales Servicompra","$ Cobrados en ServiCompras con Wallet"]
)
df_PenRedPago.fillna({"Semana":"TOTAL"}, inplace=True)

tasa = (df_PenRedPago.loc["colTOTAL","$ Cobrados en ServiCompras con Wallet"] /
        df_PenRedPago.loc["colTOTAL","$ Totales Servicompra"])
df_PenRedPago.fillna({"Penetracion %":tasa}, inplace=True)


df_PenRedPago=df_PenRedPago.reindex(columns=['Semana','$ Cobrados en ServiCompras con Wallet','$ Totales Servicompra','Penetracion %'])
...
numCols0 = ["$ Cobrados en ServiCompras con Wallet"
            , "$ Totales Servicompra"
...
df_PenRedPago = _estiladorVtaTituloP(df_PenRedPago,numCols0,numCols,percColsPen, "RedPago ServiCompra - Penetracion en $ de Operaciones")



ubicacion = "C:/Informes/Red Pago//"
nombreDespachos = "PenRedPagoServiP.png"
