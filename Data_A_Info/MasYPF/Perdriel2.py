...
presuMix = presuMix[presuMix['UEN']=='PERDRIEL2           ']
presuMix = presuMix.convert_dtypes()
##########################################
################ MENSUAL ACUMULADO  ######
##########################################

##########################   DESPACHOS ACUMULADOS MENSUAL PERDRIEL
...
p1.fillna({"Responsables":'Gonzalez'}, inplace=True)
...
p1.fillna({"Responsables":'Diaz'}, inplace=True)
############################################# PENETRACION APP YPF PLAYA #############################################

df_VentasYPF = df_VentasYPFAPPREPORTE[df_VentasYPFAPPREPORTE['UEN']=='PERDRIEL2           ']
...
p1=p1.reset_index()
p1=p1.rename(columns={'index':'Perdriel 2'})
...
p1 = p1.drop(p1[p1['Perdriel 2'] == 'Volumen Naftas'].index)
p1 = p1.drop(p1[p1['Perdriel 2'] == 'Volumen Infinia Diesel'].index)

p1.loc["colTOTAL"]= pd.Series(
    p1.sum()
    , index=['Premio']
)
p1.fillna({"Perdriel 2":"Total"}, inplace=True)
p1.loc["% de Ejecución"]= pd.Series()
p1.fillna({"Perdriel 2":"% de Ejecución"}, inplace=True)
optimo=(p1.loc['colTOTAL','Premio']/250)
p1.fillna({"Premio":optimo}, inplace=True)
p1=p1.fillna(' ')
p1=p1.reindex(columns=['Perdriel 2','Responsables','Objetivo','Ejecucion','Desvio','Premio'])
p1 = p1.astype(str)
...
p1 = _estiladorVtaTituloD(p1,numCols,percColsPen, "Programa +YPF Perdriel 2")



ubicacion = "C:/Informes/MasYPF/"
nombrePen = "+YPFPerdriel2.png"
