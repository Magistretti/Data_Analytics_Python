...
presuMix = presuMix[presuMix['UEN']=='PERDRIEL            ']
presuMix = presuMix.convert_dtypes()
##########################################
################ MENSUAL ACUMULADO  ######
##########################################
...
p1.fillna({"Responsables":'Cadile'}, inplace=True)
...
df_VentasYPF = df_VentasYPFAPPREPORTE[df_VentasYPFAPPREPORTE['UEN']=='PERDRIEL            ']
...
p1=p1.reset_index()
p1=p1.rename(columns={'index':'Perdriel 1'})
...
p1 = p1.drop(p1[p1['Perdriel 1'] == 'Volumen Naftas'].index)
p1 = p1.drop(p1[p1['Perdriel 1'] == 'Volumen Infinia Diesel'].index)

p1.loc["colTOTAL"]= pd.Series(
    p1.sum()
    , index=['Premio']
)
p1.fillna({"Perdriel 1":"Total"}, inplace=True)
p1.loc["% de Ejecución"]= pd.Series()
p1.fillna({"Perdriel 1":"% de Ejecución"}, inplace=True)
optimo=(p1.loc['colTOTAL','Premio']/250)
p1.fillna({"Premio":optimo}, inplace=True)
p1=p1.fillna(' ')
p1=p1.reindex(columns=['Perdriel 1','Responsables','Objetivo','Ejecucion','Desvio','Premio'])
p1 = p1.astype(str)
...
p1 = _estiladorVtaTituloD(p1,numCols,percColsPen, "Programa +YPF Perdriel 1")
