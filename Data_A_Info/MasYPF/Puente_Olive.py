...
presuMix = presuMix[presuMix['UEN']=='PUENTE OLIVE        ']
presuMix = presuMix.convert_dtypes()
##########################################
################ MENSUAL ACUMULADO  ######
##########################################

##########################   DESPACHOS ACUMULADOS MENSUAL PUENTE OLIVE
...
p1.fillna({"Responsables":'Cerón'}, inplace=True)
...
df_VentasYPF = df_VentasYPFAPPREPORTE[df_VentasYPFAPPREPORTE['UEN']=='PUENTE OLIVE        ']
...
p1=p1.reset_index()
p1=p1.rename(columns={'index':'Puente Olive'})
...
p1 = p1.drop(p1[p1['Puente Olive'] == 'Volumen Naftas'].index)
p1 = p1.drop(p1[p1['Puente Olive'] == 'Volumen Infinia Diesel'].index)

p1.loc["colTOTAL"]= pd.Series(
    p1.sum()
    , index=['Premio']
)
p1.fillna({"Puente Olive":"Total"}, inplace=True)
p1.loc["% de Ejecución"]= pd.Series()
p1.fillna({"Puente Olive":"% de Ejecución"}, inplace=True)
optimo=(p1.loc['colTOTAL','Premio']/250)
p1.fillna({"Premio":optimo}, inplace=True)
p1=p1.fillna(' ')
p1=p1.reindex(columns=['Puente Olive','Responsables','Objetivo','Ejecucion','Desvio','Premio'])
p1 = p1.astype(str)
...
p1 = _estiladorVtaTituloD(p1,numCols,percColsPen, "Programa +YPF Puente Olive")
