...
presuMix = presuMix[presuMix['UEN']=='AZCUENAGA           ']
presuMix = presuMix.convert_dtypes()
##########################################
################ MENSUAL ACUMULADO  ######
##########################################

##########################   DESPACHOS ACUMULADOS MENSUAL AZCUENAGA
...
p1.fillna({"Responsables":'Rodriguez'}, inplace=True)
...
df_VentasYPF = df_VentasYPFAPPREPORTE[df_VentasYPFAPPREPORTE['UEN']=='AZCUENAGA           ']
...
p1=p1.reset_index()
p1=p1.rename(columns={'index':'Azcuenaga'})
...
p1 = p1.drop(p1[p1['Azcuenaga'] == 'Volumen Naftas'].index)
p1 = p1.drop(p1[p1['Azcuenaga'] == 'Volumen Infinia Diesel'].index)

p1.loc["colTOTAL"]= pd.Series(
    p1.sum()
    , index=['Premio']
)
p1.fillna({"Azcuenaga":"Total"}, inplace=True)
p1.loc["% de Ejecución"]= pd.Series()
p1.fillna({"Azcuenaga":"% de Ejecución"}, inplace=True)
optimo=(p1.loc['colTOTAL','Premio']/250)
p1.fillna({"Premio":optimo}, inplace=True)
p1=p1.fillna(' ')
p1=p1.reindex(columns=['Azcuenaga','Responsables','Objetivo','Ejecucion','Desvio','Premio'])
p1 = p1.astype(str)
...
p1 = _estiladorVtaTituloD(p1,numCols,percColsPen, "Programa +YPF Azcuenaga")


ubicacion = "C:/Informes/MasYPF/"
nombrePen = "+YPFAzcuenaga.png"
