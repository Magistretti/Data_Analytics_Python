...
presuMix = presuMix[presuMix['UEN']=='SAN JOSE            ']
presuMix = presuMix.convert_dtypes()
##########################################
################ MENSUAL ACUMULADO  ######
##########################################

##########################   DESPACHOS ACUMULADOS MENSUAL SAN JOSE
...
p1.fillna({"Responsables":'Ulloa'}, inplace=True)
...
df_VentasYPF = df_VentasYPFAPPREPORTE[df_VentasYPFAPPREPORTE['UEN']=='SAN JOSE            ']
...
p1=p1.reset_index()
p1=p1.rename(columns={'index':'San Jose'})
...
p1 = p1.drop(p1[p1['San Jose'] == 'Volumen Naftas'].index)
p1 = p1.drop(p1[p1['San Jose'] == 'Volumen Infinia Diesel'].index)

p1.loc["colTOTAL"]= pd.Series(
    p1.sum()
    , index=['Premio']
)
p1.fillna({"San Jose":"Total"}, inplace=True)
p1.loc["% de Ejecución"]= pd.Series()
p1.fillna({"San Jose":"% de Ejecución"}, inplace=True)
optimo=(p1.loc['colTOTAL','Premio']/250)
p1.fillna({"Premio":optimo}, inplace=True)
p1=p1.fillna(' ')
p1=p1.reindex(columns=['San Jose','Responsables','Objetivo','Ejecucion','Desvio','Premio'])
p1 = p1.astype(str)
...
p1 = _estiladorVtaTituloD(p1,numCols,percColsPen, "Programa +YPF San Jose")
