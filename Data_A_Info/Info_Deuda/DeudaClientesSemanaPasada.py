...
(SELECT vta.NROCLIENTE,sum(vta.importe) as 'saldo Cuenta',CLI.NOMBRE,CLI.LIMITECREDITO
  ...
  where vta.NROCLIENTE > '100000'
   ...
   and vta.FECHASQL <= DATEADD(day, -8, CAST(GETDATE() AS date))
...
deudaPendFactu = pd.read_sql( '''
		...
		where vta.NROCLIENTE > '100000'
		and rem.FECHASQL <= DATEADD(DAY,-8,CAST(GETDATE() AS date)) ) as e
		where e.FECHASQLFAC  > DATEADD(DAY,-8,CAST(GETDATE() AS date))
...
deudaPendFactum = pd.read_sql( '''
        ...
        AND NROCOMP = 0
        AND IMPTOTAL > 0
        AND FECHASQL <= DATEADD(DAY,-8,CAST(GETDATE() AS date))
...
SELECT
            ...
            where FRD.NROCLIENTE > '100000'
                and Cli.ListaSaldoCC = 1
                and FECHASQL >= DATEADD(day, -68, CAST(GETDATE() AS date))
				and FECHASQL <= DATEADD(day, -8, CAST(GETDATE() AS date))
...
ultimacompra = pd.read_sql(
 ''' 
   SELECT
            ...
            , DATEDIFF(DAY, MAX(CAST(FRD.[FECHASQL] as date)), DATEADD(DAY,-8,CAST(GETDATE() AS date))) as 'Días Desde Última Compra'
            FROM [Rumaos].[dbo].[FacRemDet] as FRD with (NOLOCK)

            where FRD.NROCLIENTE > '100000'
                and FECHASQL >= '20140101' and FECHASQL <= DATEADD(DAY,-8,CAST(GETDATE() AS date))
...
precioGO = pd.read_sql( '''
    select uen,PRECARTEL from EmpVenta where FECHASQL = DATEADD(DAY,-8,CAST(GETDATE() AS date))
    and CODPRODUCTO = 'GO'
    and UEN = 'PERDRIEL'
...
nombreT='Deuda Comercial Semana.png'
