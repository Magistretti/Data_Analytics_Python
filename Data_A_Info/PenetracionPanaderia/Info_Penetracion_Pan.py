...
df_despachosM = pd.read_sql('''
	...
	SELECT 
       ...
       D.UEN,count(D.ID) as 'DESPACHOS',D.TURNO
...
df_despachos = pd.read_sql('''
    ...
    WHERE 
		 ...
		 D.FECHASQL = DATEADD(day, -1, CAST(GETDATE() AS date))
        AND D.UEN NOT IN ('AZCUENAGA           '
        , 'LAMADRID            '
        , 'PUENTE OLIVE        '
        ,'SAN JOSE            ')
...
tasa6 = ((penetracionPanT.loc["colTOTAL","PenetracionT1"] /
    penetracionPanT.loc["colTOTAL","Presupuesto"])-1)
...
ARGS:
    ...
    resultado = resultado.background_gradient(
        ...
        ,vmin=-0.02
        ,vmax=0.04
        ,subset=pd.IndexSlice[evitarTotales[:-1],"Penetracion Total Diaria"]
    )
    resultado = resultado.background_gradient(
    ...
    resultado = resultado.background_gradient(
        ...
        ,vmin=-0.02
        ,vmax=0.04
        ,subset=pd.IndexSlice[evitarTotales[:-1],"Desvio Total Diario"]
    )
...
def _estiladorVtaTitulo(df, list_Col_Num, list_Col_Perc, titulo):
    ...
    resultado = df.style \
        ...
        .background_gradient(cmap="RdYlGn",
                             vmin=presupuesto_min,
                             vmax=presupuesto_max,
                             subset=["Penetracion Total Acumulada"]) \
        .background_gradient(cmap="RdYlGn",
                             vmin=presupuesto_min,
                             vmax=presupuesto_max,
                             subset=["Penetracion Total Diaria"]) \
        .apply(lambda x: ["background: black" if x.name == "colTOTAL" 
            else "" for i in x]
            , axis=1) \
        .apply(lambda x: ["color: white" if x.name == "colTOTAL" 
...
percColsPen = ["Penetracion Diaria TURNO 1"
	,"Penetracion Diaria TURNO 2"
    ,"Penetracion Total Diaria"
	,"Presupuesto Diario"
    ,"Penetracion Acumulada TURNO 1"
...
percColsDiaria = ["Penetracion Diaria TURNO 1"
	,"Penetracion Diaria TURNO 2"
    ,'Penetracion Total Diaria'
	,"Presupuesto Diario"
	,"Desvio Diario TURNO 1"
...
#penetracionPanT = _estiladorVtaTitulo(penetracionPanT, numCols, percCols, "INFO PENETRACION PANADERIA")
penetracionPanDiario = _estiladorVtaTituloD(penetracionPanDiario,numCols,percColsDiaria, "EJECUCION PANADERIA PRESUPUESTADO DIARIO")
...
nombrePen = "Info_Penetracion_Pan.png"
