...
presuLubri =pd.read_excel(ubicacion+aux_semanal, sheet_name= 'Hoja1')
...
penetracionLubriOnly.fillna({"Presupuesto Mensual":0.0117}, inplace=True)
...
penetracionLubriOnly.fillna({"Presupuesto Mensual":0.020125}, inplace=True)
...
penetracionDiaria.fillna({"Presupuesto Mensual":0.0117}, inplace=True)
...
penetracionDiaria.fillna({"Presupuesto Mensual":0.020125}, inplace=True)
...
ARGS:
    ...
    resultado = resultado.background_gradient(
        ...
        ,vmin=0.001
        ,vmax=0.007
        ,subset=pd.IndexSlice[evitarTotales[:-1],"Penetracion Total Acumulada"]
    )
    resultado = resultado.background_gradient(
...
ARGS:
    ...
    resultado = resultado.background_gradient(
        ...
        ,vmin=-0.4
        ,vmax=0.1
        ,subset=pd.IndexSlice[evitarTotales[:-1],"Desvio Diario Total"]
    )
    return resultado
...
nombrePen = "Info_PenetracionLubri_Diario.png"
